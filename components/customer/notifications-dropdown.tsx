...
import { IconArrowDown, IconBell, IconClose } from "../icons";
...
export const CustomerNotificationsDropdown = () => {
    ...
    const { data, isLoading } = useGetCustomerNotifications({ page: String(page), item_per_page: String(ITEMS_PER_PAGE) })
    const { data: messagesCount, isLoading: isLoadingMessagesCount } = useGetCustomerNotifications({ component: "count" })
    const { data: unreadMessagesCount, isLoading: isLoadingUnreadMessages } = useGetCustomerNotifications({ component: "count-unread" })
    const viewportRef = useRef<HTMLDivElement | null>(null)
    const [isBottomVisible, setIsBottomVisible] = useState(true);

    const total = (messagesCount?.data as GetNotificationCountResponse)?.total ?? 0;
    ...
    return (
        ...
        <Popover>
            <div className="relative">
                {
                    (unreadMessagesCount?.data as GetNotificationCountResponse)?.total > 0 && (
                        <span className="absolute -top-0.5 -right-0.5 z-1 text-xs font-medium text-white rounded-full bg-error p-px px-1.5">{(unreadMessagesCount?.data as GetNotificationCountResponse).total}</span>
                    )
                }
            ...
            <PopoverContent className="p-0! w-100 origin-top-right gap-0 max-h-149.5" align="end" sideOffset={8}>
                <div className="flex items-center justify-between py-4 px-5 bg-input-field rounded-t-2xl">
                    <PopoverHeader>
                        <PopoverTitle className="flex items-center gap-1 font-semibold text-base text-grey-dark-0">Notifications <span className="text-xs font-medium text-white rounded-full bg-error p-px px-1.5">{(unreadMessagesCount?.data as GetNotificationCountResponse)?.total}</span></PopoverTitle>
                    </PopoverHeader>
                    <div className="flex items-center justify-end gap-4">
                        <div className="flex items-center gap-2">
                            <Button size="icon-sm" variant="outline" className="bg-grey-dark-4" disabled={!hasPrev || isLoading} onClick={() => setPage((p) => Math.max(p - 1, 1))}>
                                <IconArrowDown className="rotate-90" />
                            </Button>
                            <Button size="icon-sm" variant="outline" className="bg-grey-dark-4" disabled={!hasNext || isLoading} onClick={() => setPage((p) => Math.min(p + 1, totalPages))}>
                                <IconArrowDown className="-rotate-90" />
                            </Button>
                        </div>
                ...
                {
                    (isLoading || isLoadingUnreadMessages || isLoadingMessagesCount || !data) ? (
                        ...
                        <ScrollArea viewportRef={viewportRef} className={cn("relative overscroll-contain overflow-scroll flex-1 h-[calc(598px-64px)]", !isBottomVisible ? "mask-alpha mask-b-from-75% mask-b-from-[#FF0000] mask-b-to-100% mask-b-to-[#FF0000]/0" : "")}>
                            ...
                            {
                                (data.data as GetVendorNotificationResponse[]).map((notification) => (
