...
import { useLoginCustomer } from "@/services/mutations/use-auth";
...
export const CustomerLoginContent = () => {
    ...
    const customerLoginForm = useForm({
        defaultValues: {
            email: "",
            password: "",
            rememberMe: false
    ...
    return (
        <form className="flex flex-col gap-12.5 w-full max-w-100.5 mx-auto" onSubmit={(e) => {
            e.preventDefault()
            customerLoginForm.handleSubmit()
        }}>
            <div className="text-center space-y-3 w-full max-w-153 mx-auto">
                ...
                <p className="text-sm text-grey-dark-3">Don’t have an account? <Link href="/customer/register" className="font-medium text-grey-dark-0 hover:underline hover:underline-offset-1">Sign up instead</Link></p>
            </div>

            <FieldGroup>
                <customerLoginForm.Field name="email">
                    {(field) => {
                ...
                </customerLoginForm.Field>
                
                <customerLoginForm.Field name="password">
                    {(field) => {
                        const isInvalid = !field.state.meta.isValid
                        return (
                            <Field data-invalid={isInvalid}>
                ...
                <customerLoginForm.Field name="rememberMe">
                    {(field) => {
                        const isInvalid = !field.state.meta.isValid
                        return (
                            <Field data-invalid={isInvalid}>
            ...
            <div className="flex flex-col items-center gap-5">
                ...
                <Link href="/customer/forgot-password" className="font-medium text-sm text-grey-dark-0 hover:underline hover:underline-offset-1">Reset Password</Link>
